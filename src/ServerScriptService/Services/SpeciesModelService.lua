...
function SpeciesModelService:ResolveModel(speciesId, growthStage)
...
function SpeciesModelService:ValidateConfiguredModels()
    local failures = {}
    for speciesId in pairs(SpeciesConfig) do
        for _, stage in ipairs(self.RequiredStages) do
            local model, reason = self:ResolveModel(speciesId, stage)
            if not model then
