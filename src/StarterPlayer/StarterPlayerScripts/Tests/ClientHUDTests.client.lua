...
table.insert(suite.tests, { name = "stat delta text makes progression readable", run = function()
    ...
    Assert.truthy(string.find(delta, "Zoom uses energy", 1, true) ~= nil, "sprint drain hint shown")
end })
...
table.insert(suite.tests, { name = "diet guidance explains species food and water", run = function()
    ...
    Assert.truthy(string.find(herbivore, "green plants", 1, true) ~= nil, "herbivore food hint appears")
    ...
    Assert.truthy(string.find(carnivore, "red meat", 1, true) ~= nil, "carnivore food hint appears")
end })
