...
| US04 | Players cannot drink/eat before hatching. | FAIL | `FoodWaterService:RequestDrink` checks alive and range/tag, but does not require hatched state. | Require hatched state for drinking. |
...
| US07 | Combat applies server-authoritative health/damage, not only a pending attribute. | FAIL | `CombatService.lua` currently sets only `PendingServerDamage` on targets. | Apply validated damage to target health/NPC state. |
