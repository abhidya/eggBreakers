...
| --- | --- | --- | --- | --- |
| Beat 0 — Egg wakeup | Baby dinosaur mesh, nest/egg visual, no default avatar/helper box | `StoryboardBeatValidation.server`: staged hatchling mesh + hidden helpers; imported egg/nest when source exists | Covered in source | Screenshot must prove real baby dino, real nest/egg, safe food/water, no default avatar |
