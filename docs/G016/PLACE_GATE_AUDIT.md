...
```sh
lune run tools/g016_place_gate_audit.luau eggBreakers.rbxl eggBreakers2.rbxl eggBreakers3.rbxl eggBreakers4.rbxl eggBreakers5.rbxl eggBreakers6.rbxl eggBreakers7.rbxl
```

## Result

The persisted `.rbxl` files do not prove G016 complete. The best persisted
candidate is `eggBreakers7.rbxl`: it starts from the cleaned
`eggBreakers6.rbxl` baseline, then uses historical Git `.rbxl` blobs plus
...
| `eggBreakers7.rbxl` | 13/15 | 295 | 205 | 263 | 0 | false | false | FAIL |

## Current Blockers

- US14 and US15 live proof are absent in every persisted `.rbxl`.
- `eggBreakers7.rbxl` only proves 295 unique release-ready source ids under the
  production audit roots, not the required 500. Its 263 broad-world ids are
...
- Earlier persisted candidates still contain validation/test leftovers such as
  ...
  `TestImported*` roots. `eggBreakers5.rbxl`, `eggBreakers6.rbxl`, and
  `eggBreakers7.rbxl` remove those from their source backups and keep
  `tempArtifactCount=0`.
